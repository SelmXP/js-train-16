...
// warnAboutMaxArrayLength([1, 2, 3, 4, 5], 5);
...
function compareArrays(arr1, arr2) {
  ...
  const compare_arr = JSON.stringify(arr1) === JSON.stringify(arr2);
  // Використовуємо `console.assert()`, щоб вивести помилку, якщо масиви не однакові.
  return console.assert(compare_arr, "Масиви не однакові");
}
...
function buildMatrix(size) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    for (let j = 0; j < size; j++) {
      ...
      arr_row[j] = Math.floor(Math.random() * 10);
    }
    // Додаємо рядок до матриці.
    // build_matrix[i] = arr_row;
    build_matrix.push(arr_row);
  }
  // Виводимо матрицю в консоль за допомогою як таблицю.
  console.table(build_matrix);
...
function calculateTotalPrice(products) {
  ...
  for (const rec of products) total += rec.price;
...
function countVowelsAndConsonants(word) {
  ...
  for (let letter of word) {
    ...
    letter_v.includes(letter.toLowerCase())
